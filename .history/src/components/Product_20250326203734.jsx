...
const Product = ({ rating = 4.5, totalReviews = 738 }) => {
  return (
    <div className="w-[234px] h-[320px] border rounded-lg shadow-md bg-white relative p-3 group">
      {/* HOT Badge (Top-left) */}
      <div className="absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-md z-">
        HOT
      </div>
...
const IconButton = ({ icon }) => {
  return (
    <div className="bg-white p-2 rounded-full shadow-md hover:bg-gray-200 transition duration-300 cursor-pointer">
      {icon}
    </div>
  );
};
